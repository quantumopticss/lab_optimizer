...
class gaussian_process(agent_model):
    def __init__(self,kernel_type = "general", noise_level = 1.0,save = True):
        ...
        C0 = ConstantKernel(1.0, (1e-5,1e3))
        match kernel_type:
            case "smooth":
                kernel = C*RBF(length_scale = 1.0) + WhiteKernel(noise_level=noise_level, noise_level_bounds = (1e-5,1e3)) + C0
            case "periodic":
                kernel = C*ExpSineSquared(length_scale = 1.0,periodicity = 1.0) + WhiteKernel(noise_level=noise_level, noise_level_bounds = (1e-5,1e3)) + C0
            case "nonlinear":
                kernel = C*Matern(length_scale = 1.0,nu = 2.5) + C0 + WhiteKernel(noise_level=noise_level, noise_level_bounds = (1e-5,1e3))
            case _:
                kernel = C*(RBF(length_scale = 1.0) + Matern(length_scale = 1.0,nu = 1.5)) + C0 + WhiteKernel(noise_level=noise_level, noise_level_bounds = (1e-5,1e3))
    ...
    def save(self,pkl_path:str = None):
        super().save(self.model,pkl_path,decstr="gaussian_process")
